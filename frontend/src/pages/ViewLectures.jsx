  ...
  return (
    <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
      ...
      <div className='w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200'>
        <div className='mb-6'>
          <h2 className='text-2xl font-bold flex items-center justify-start gap-[20px] text-gray-800'><FaArrowLeftLong className='text-black w-[22px] h-[22px] cursor-pointer' onClick={()=>navigate("/home")}/>{selectedCourse?.title}</h2>
        ...
        <div className='aspect-video bg-black rounded-xl overflow-hidden mb-4 border border-gray-300'>
          {selectedLecture?.videoUrl ? <video className='w-full h-full object-cover' src={selectedLecture?.videoUrl} controls/>:<div className='flex items-center justify-center h-full text-white'>Select a lecture to start watching</div>}
        </div>
      ...
      <div className='w-full md:w-1/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200 h-fit'>
        ...
        <div className='flex flex-col gap-3 mb-6'>
          ...
          (selectedCourse?.lectures?.map((lecture, index)=>(
              ...
              }`}><h2 className='text-sm font-semibold text-gray-800'>{lecture.lectureTitle}</h2><FaPlayCircle className='text-lg text-black' /></button> 
          )))
        ...
        { creatorData &&
          <div className='mt-4 border-t pt-4'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-600'>{creatorData?.name}</p>
